package ru.mipt.cybersecurity.cripto;
...
/**
 * The idea (and this annotation) of {@code Cipher} is taken from org.apache.shiro.crypto.CipherService.
 * https://shiro.apache.org/
 *
 * A {@code Cipher} uses a cryptographic algorithm called a
 ...
 * a {@code key} as well.  {@code Cipher}s can perform both encryption and decryption.
 * <h2>Cipher Basics</h2>
 ...
 * <h2>CyberSecurity {@code Cipher}s vs JDK {@link javax.crypto.Cipher Cipher}s</h2>
 * Shiro {@code Cipher}s essentially do the same things as JDK {@link javax.crypto.Cipher Cipher}s, but in
 * simpler and easier-to-use ways for most application developers.  When thinking about encrypting and decrypting data
 * in an application, most app developers want what a {@code Cipher} provides, rather than having to manage the
 * lower-level intricacies of the JDK's {@code Cipher} API.  Here are a few reasons why most people prefer
 * {@code Cipher}s:
 * <ul>
 * <li><b>Stateless Methods</b> - {@code Cipher} method calls do not retain state between method invocations.
 ...
 * <li><b>Thread Safety</b> - {@code Cipher} instances are thread-safe inherently because no state is
 ...
 * <li><b>Single Operation</b> - {@code Cipher} method calls are single operation methods: encryption or
 ...
 * <li><b>Type Safe</b> - There are {@code Cipher} implementations for different Cipher algorithms
 * ({@code AesCipher}, {@code BlowfishCipher}, etc).  There is only one JDK {@code Cipher} class to
 * represent all cipher algorithms/instances.
 * <li><b>Simple Construction</b> - Because {@code Cipher} instances are type-safe, instantiating and using
 * one is often as simple as calling the default constructor, for example, <code>new AesCipher();</code>.  The
...
public interface Cipher {
     ...
     *     InputStream in = ...
     *     OutputStream out = ...
     *     cipher.decrypt(in, out, decryptionKey);
     * } finally {
     *     if (in != null) {
     *         try {
     *             in.close();
     ...
     *     InputStream in = ...
     *     OutputStream out = ...
     *     cipher.encrypt(in, out, encryptionKey);
     * } finally {
     *     if (in != null) {
     *         try {
     *             in.close();
