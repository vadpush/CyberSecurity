package ru.mipt.cybersecurity.jcajce.provider.keystore.bcfks;
...
import ru.mipt.cybersecurity.asn1.bc.EncryptedPrivateKeyData;
import ru.mipt.cybersecurity.asn1.bc.EncryptedSecretKeyData;
import ru.mipt.cybersecurity.asn1.bc.ObjectData;
import ru.mipt.cybersecurity.asn1.bc.ObjectDataSequence;
import ru.mipt.cybersecurity.asn1.bc.ObjectStore;
import ru.mipt.cybersecurity.asn1.bc.ObjectStoreData;
import ru.mipt.cybersecurity.asn1.bc.ObjectStoreIntegrityCheck;
import ru.mipt.cybersecurity.asn1.bc.PbkdMacIntegrityCheck;
import ru.mipt.cybersecurity.asn1.bc.SecretKeyData;
import ru.mipt.cybersecurity.asn1.cms.CCMParameters;
...
import ru.mipt.cybersecurity.asn1.pkcs.EncryptionScheme;
...
import ru.mipt.cybersecurity.crypto.PBEParametersGenerator;
import ru.mipt.cybersecurity.crypto.digests.SHA3Digest;
import ru.mipt.cybersecurity.crypto.digests.SHA512Digest;
import ru.mipt.cybersecurity.crypto.generators.PKCS5S2ParametersGenerator;
    ...
    extends KeyStoreSpi
{
    ...
    public Certificate[] engineGetCertificateChain(String alias)
    {
        ...
        if (ent != null)
        {
            ...
            {
                EncryptedPrivateKeyData encPrivData = EncryptedPrivateKeyData.getInstance(ent.getData());
                ru.mipt.cybersecurity.asn1.x509.Certificate[] certificates = encPrivData.getCertificateChain();
    ...
    private Certificate decodeCertificate(Object cert)
    {
        if (provider != null)
        {
            try
            {
                ...
                return certFact.generateCertificate(new ByteArrayInputStream(ru.mipt.cybersecurity.asn1.x509.Certificate.getInstance(cert).getEncoded()));
            }
            catch (Exception e)
            {
                return null;
    ...
    {
        ru.mipt.cybersecurity.asn1.x509.Certificate[] certChain = new ru.mipt.cybersecurity.asn1.x509.Certificate[chain.length];
        for (int i = 0; i != chain.length; i++)
        {
            certChain[i] = ru.mipt.cybersecurity.asn1.x509.Certificate.getInstance(chain[i].getEncoded());
        }
