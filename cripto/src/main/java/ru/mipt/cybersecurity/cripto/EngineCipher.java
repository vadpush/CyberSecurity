...
//ToDo Redo to the engines from Jca
public abstract class EngineCipher implements Cipher{
    /**
     * Internal private log instance.
     */
    private static final Logger log = LoggerFactory.getLogger(EngineCipher.class);
     ...
     */
    protected EngineCipher(String algorithmName) {
    ...
    protected byte[] generateInitializationVector(boolean streaming) {
        ...
        if (size <= 0) {
            String msg = "initializationVectorSize property must be greater than zero.  This number is " +
                    "typically set in the " + Cipher.class.getSimpleName() + " subclass constructor.  " +
